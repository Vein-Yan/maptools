<template>
  <div>
    ...
    <section class="v-settings" :class="{ hide: filterType !== 'province' }">
      <spatial-filter-form
        :lonColumns="lonColumns"
        :selectedLon="selectedLontitude"
        :latColumns="latColumns"
        :selectedLat="selectedLattitude"
        :allData="allProvinces"
        type="省份"
        @onSettingChange="handleProvinceSettingChange"
      ></spatial-filter-form>
    </section>
    <section class="v-settings" :class="{ hide: filterType !== 'city' }">
      <spatial-filter-form
        ...
        :allData="allCities"
        type="城市"
        @onSettingChange="handleCitySettingChange"
      ></spatial-filter-form>
    </section>
    <section class="v-settings" :class="{ hide: filterType !== 'customize' }">
      <spatial-filter-form
        ...
        :selectedLat="selectedLattitude"
        type="区域"
        @onSettingChange="handleCustomSettingChange"
      ></spatial-filter-form>
      <el-upload
        v-else
        class="upload-demo"
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip">
          上传自定义的区域数据geojson文件，可在绘制图形模块绘制并导出
        </div>
      </el-upload>
    </section>
  </div>
